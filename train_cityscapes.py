...
def train(args, model, optimizer, dataloader_train, dataloader_val,load_epoch):
    ...
    for epoch in range(load_epoch,args.num_epochs):
        ...
        if epoch % args.validation_step == 0 and epoch != 0:
            ...
            }, './checkpoints/cityscapes/checkpoint_cityscapes.tar')
            if miou > max_miou:
                max_miou = miou
                import os
...
def main():
    ...
    if is_folder_not_empty('./checkpoints/cityscapes'):
        checkpoint = torch.load('./checkpoints/cityscapes/checkpoint_cityscapes.tar')
